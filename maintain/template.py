...
def check_empty(item: dict, key_str: str):
    '''检查对应配置项是否为空'''
    if item[key_str] is None or item[key_str] == '':
        st.toast(f'{key_str}为空', icon='🚨', duration='infinite')
        return False
    return True
...
def detail_view():
    ...
    with st.container(horizontal=True):
        ...
        if st.session_state['page_item']['设计方案项'] in total_configuration:
            st.session_state['page_item']['设计方案项'] = st.selectbox('设计方案项', get_total_configuration(), total_configuration.index(st.session_state['page_item']['设计方案项']))
        else:
            st.session_state['page_item']['设计方案项'] = st.selectbox('设计方案项', get_total_configuration())
    st.text('模板的工步分录')
    if 1:
...
if st.session_state['page_path'] == "main":
    detail_view()
